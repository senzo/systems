...
Raphael.fn.connection = function (obj1, obj2, line, bg) {
    ...
    var bb1 = obj1.getBBox(), //BBox = dimension d un element 
        bb2 = obj2.getBBox(),
        ...
        {x: bb2.x + bb2.width + 1, y: bb2.y + bb2.height / 2}], //tableau p avec des x et y 
        d = {}, dis = []; // 
    for (var i = 0; i < 4; i++) {
        for (var j = 4; j < 8; j++) {
            ...
            if ((i == j - 4) || (((i != 3 && j != 6) || p[i].x < p[j].x) && ((i != 2 && j != 7) || p[i].x > p[j].x) && ((i != 0 && j != 5) || p[i].y > p[j].y) && ((i != 1 && j != 4) || p[i].y < p[j].y))) {
                dis.push(dx + dy); // on met la valeur dans le tableau dis
                d[dis[dis.length - 1]] = [i, j];
            }
        } // on met les points de la forem 
    }
    if (dis.length == 0) {
        var res = [0, 4];
    } else {
